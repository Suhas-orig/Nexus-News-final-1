...
function News3() {
  ...
  useEffect(() => {
    fetch("http://localhost:8000/scraper3/scrape/")
      .then(res => res.json())
      .then(data => {
      ...
      .catch(err => {
        console.error("Error fetching The Hindu news", err);
        setLoading(false);
      });
  }, []);

  if (loading) return <VintageLoader />;  // Early return for loading state
